...
Spin up a functioning web server by including `http_platform::default`.
...
The workflow for doing so is as below.
...
* On the first Chef run, the server will be configured to use the SS cert because path_to_ssl_cert and path_to_ssl_key point to SS credentials.
...
Certbot is necessarily run in non-interactive mode and the [Let's Encrypt Terms of Service](https://letsencrypt.org/repository/) are accepted as part of this.
Therefore, use of this recipe constitutes accepting the [Let's Encrypt Terms of Service](https://letsencrypt.org/repository/).
